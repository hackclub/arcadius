...
export async function getDmChannelFromAirtable(params: Identifier ) {
 ...
 let identifierType: 'slackId' | 'email' | 'internalId' | 'unknown' = 'unknown';
 ...
 let dmChannel;
  ...
  switch (identifierType) {
    ...
    case 'slackId':
      ...
      userRecord = await hoursAirtable.select({ filterByFormula: `{Slack ID} = '${slackId}'`, pageSize: 1 }).firstPage();
    ...
    case 'email':
      ...
      userRecord = await hoursAirtable.select({ filterByFormula: `{Email} = '${email}'`, pageSize: 1 }).firstPage();
      dmChannel = userRecord[0].get('dmChannel');
    ...
    case 'internalId':
      ...
      userRecord = await hoursAirtable.select({ filterByFormula: `{Internal ID} = '${internalId}'`, pageSize: 1 }).firstPage();
      dmChannel = userRecord[0].get('dmChannel');
