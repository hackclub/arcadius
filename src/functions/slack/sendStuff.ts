...
async function sendInitalDM(userId) {
    try {
        metrics.increment("http.request.api_chat-postmessage.200");
        ...
        metrics.increment("slack.send_inital_dm.500");
    }
}
...
async function postRacoonInitalInstructions(payload) {
    try {
        ...
        metrics.increment("slack.post_racoon_inital_instructions.500");
    }
}
...
async function checkOutTheShop(userId) {
    try {
        ...
        metrics.increment("slack.check_out_the_shop.500");
    }
}
...
async function sendAlreadyVerifiedDM(userId) {
    try {
        ...
        metrics.increment("slack.send_already_verified_dm.500");
    }
}
...
async function sendUpgradedDM(userId) {
    try {
        ...
        metrics.increment("slack.send_upgraded_dm.500");
    }
}
