...
export async function pollFirstPurchaseUsers() {
    try {
        metrics.increment("polling.poll_firstpurchaseusers.200");

        blog(
            "Polling for users who have just made their first purchase",
            "info"
        );
        ...
        console.log(
            colors.bgBlack(
                `Users who have just made their first purchase: ${users}`
            )
        );
        ...
        metrics.increment("polling.poll_firstpurchaseusers.500");
    }
}
